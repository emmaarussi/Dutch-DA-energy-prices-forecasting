...
def create_generation_lagged_features(df):
    """Create lagged features for all relevant generation and forecast signals."""
    lags = [0,1, 2, 3, 6, 12, 24, 48, 168]  # 1h to weekly
    sources = ['wind', 'solar', 'consumption', 'coal', 'wind_forecast', 'solar_forecast', 'consumption_forecast']
    df = df.copy()
...
def main():
    ...
    df = pd.read_csv(os.path.join(base_dir, 'processed', 'dataset_nooutliers.csv'), index_col=0)
    ...
    print("\nSaving features...")
    df_features.to_csv(os.path.join(base_dir, 'processed', 'multivariate_features_nooutliers.csv'))

    print(f"\nFeature preparation complete. Shape: {df_features.shape}")
    print("Features available:")
    print(df_features.columns.tolist())

    cols = pd.read_csv(os.path.join(base_dir, 'processed', 'multivariate_features_nooutliers.csv'), nrows=1).columns
